...
export default function QueensPanelPage() {
  ...
  const [newReward, setNewReward] = useState({ label: '', emoji: '🎁', type: 'daily' })
  ...
  const loadData = async () => {
    const { data: athletes } = await supabase
      .from('profiles').select('*').eq('role', 'athlete').limit(1)
    ...
    const { data: todayTasks } = await supabase
      ...
      .eq('athlete_id', athlete.id).eq('date', today)
    if (todayTasks) setTasks(todayTasks)

    const { data: xpData } = await supabase
      .from('xp_log').select('xp').eq('athlete_id', athlete.id)
  ...
  const sendMessage = async () => {
    ...
    await supabase.from('messages').insert({ sender_id: user?.id, message: message.trim() })
    setMessage('')
  ...
  const sendBonusXp = async () => {
    ...
    await supabase.from('xp_log').insert({
      athlete_id: athleteId, xp: parseInt(bonusXp), reason: bonusReason,
    })
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0d0d0d] flex items-center justify-center">
      <div className="text-pink-400 text-xl font-black animate-pulse">LOADING QUEEN'S PANEL...</div>
    </div>
  )
  ...
  return (
    <main className="min-h-screen bg-[#0d0d0d] text-white pb-10">
      ...
      <div className="bg-[#111] border-b border-white/5 px-5 py-4">
        <div className="text-[10px] text-pink-400 tracking-widest">QUEST MASTER</div>
      ...
      <div className="grid grid-cols-3 gap-3 px-5 pt-4">
        ...
        <div className="bg-[#111] border border-white/5 rounded-xl p-3 text-center">
          <div className="text-xl font-black text-white">{completedCount}/5</div>
          <div className="text-[9px] text-white/30 tracking-widest mt-1">TODAY</div>
        </div>
        <div className="bg-[#111] border border-white/5 rounded-xl p-3 text-center">
          <div className="text-xl font-black text-pink-400">{pendingCount}</div>
          <div className="text-[9px] text-white/30 tracking-widest mt-1">PENDING</div>
        </div>
      </div>
      ...
      <div className="flex gap-2 px-5 pt-4 overflow-x-auto">
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            ...
            {t}{i === 1 && pendingCount > 0 ? ` (${pendingCount})` : ''}
          </button>
        ))}
      </div>
      ...
      <div className="px-5 pt-4">
        ...
        {tab === 0 && (
          <div className="flex flex-col gap-3">
            ...
            {tasks.length === 0 && (
              <div className="text-white/20 text-sm text-center py-8">No tasks logged yet today</div>
            )}
            {tasks.map(task => {
              const info = TASK_LABELS[task.task_name] || { label: task.task_name, emoji: '✅' }
              return (
                <div key={task.id}
                  className={`flex items-center gap-3 p-4 rounded-xl border ${
                    task.completed ? 'bg-green-500/10 border-green-500/20' : 'bg-[#111] border-white/5'
                  }`}>
                  <span className="text-xl">{info.emoji}</span>
                  <span className="flex-1 text-sm font-medium">{info.label}</span>
                  <span className={`text-xs font-bold ${task.completed ? 'text-green-400' : 'text-white/20'}`}>
        ...
        {tab === 1 && (
          <div className="flex flex-col gap-4">
            ...
            {/* Pending section */}
            <div className="text-[10px] text-white/30 tracking-widest">PENDING APPROVALS</div>
            {claims.filter(c => c.status === 'pending').length === 0 && (
              <div className="text-white/20 text-sm text-center py-6">No pending rewards 🎉</div>
            )}
            {claims.filter(c => c.status === 'pending').map(claim => (
              <div key={claim.id} className="bg-[#111] border border-white/5 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-bold text-white">🎁 {claim.reward_name}</span>
                  {claim.reward_type && (
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest ${TYPE_BADGE[claim.reward_type] || 'bg-white/10 text-white/40'}`}>
                      {claim.reward_type}
                    </span>
                  )}
                </div>
                <div className="text-xs text-white/30 mb-4">
                  Claimed {new Date(claim.claimed_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                </div>
                <div className="flex gap-2">
                  <button onClick={() => approveReward(claim.id, true)}
                    className="flex-1 bg-green-500/20 border border-green-500/30 text-green-400 font-bold py-2 rounded-xl text-sm">
                    ✓ Approve
                  </button>
                  <button onClick={() => approveReward(claim.id, false)}
            ...
            {claims.filter(c => c.status !== 'pending').length > 0 && (
              <>
                <div className="text-[10px] text-white/30 tracking-widest mt-2">CLAIM HISTORY</div>
                {claims.filter(c => c.status !== 'pending').map(claim => (
                  <div key={claim.id} className="bg-[#111] border border-white/5 rounded-xl p-3 flex items-center gap-3">
                    <div className="flex-1">
                      <div className="text-sm font-medium">{claim.reward_name}</div>
                      <div className="text-[10px] text-white/30 mt-0.5">
                        {new Date(claim.claimed_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                      </div>
                    </div>
        ...
        {tab === 2 && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#111] border border-white/5 rounded-2xl p-4">
              <div className="text-[10px] text-white/30 tracking-widest mb-3">SEND MOTIVATION 💬</div>
            ...
            <div className="bg-[#111] border border-white/5 rounded-2xl p-4">
              <div className="text-[10px] text-white/30 tracking-widest mb-3">GIVE BONUS XP ⚡</div>
        ...
        {tab === 3 && (
          <div className="flex flex-col gap-4">
            <div className="bg-[#111] border border-white/5 rounded-2xl p-4">
              <div className="text-[10px] text-white/30 tracking-widest mb-3">CREATE REWARD 🎁</div>
              <div className="flex gap-2 mb-2">
                <input type="text" value={newReward.emoji}
                ...
                <input type="text" value={newReward.label}
            ...
            <div className="text-[10px] text-white/30 tracking-widest">EXISTING REWARDS</div>
            {customRewards.length === 0 && (
              <div className="text-white/20 text-sm text-center py-4">No rewards added yet</div>
            )}
            {customRewards.map(r => (
              <div key={r.id} className="bg-[#111] border border-white/5 rounded-xl p-3 flex items-center gap-3">
                <span className="text-2xl">{r.emoji}</span>
                <div className="flex-1">
                  <div className="text-sm font-bold">{r.reward_name}</div>
                  <div className="text-[10px] text-white/30 uppercase tracking-widest mt-0.5">{r.reward_type}</div>
                </div>

      ...
      {[
        ...
        <div key={i} className="flex items-end gap-2">
          <div className="bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-center min-w-[52px]">
            <div
              className={`text-2xl font-black tabular-nums ${item.o ? "text-orange-500" : "text-white"}`}
            >
              {pad(item.v)}
            </div>
            <div className="text-[8px] text-white/30 tracking-widest mt-1">
              {item.l}
            </div>
          </div>
          {i < 3 && (
            <span className="text-orange-500 text-xl font-black pb-2">:</span>
          )}
        </div>
      ))}
    </div>
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#0d0d0d] flex items-center justify-center">
        <div className="text-orange-500 text-xl font-black animate-pulse">
          LOADING QUEST...
        </div>
      </div>
    );
      ...
      <div className="px-5 pt-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            {TASKS.map((task) => (
              ...
              >
                ...
                <span
                  className={`flex-1 text-sm font-medium ${tasks[task.key] ? "line-through text-white/40" : "text-white"}`}
                >
                  {task.label}
                </span>
        ...
        {allDone && (
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-5 text-center">
            ...
            <div className="font-black text-lg text-orange-500">
              REWARD UNLOCKED!
            </div>
            <div className="text-white/50 text-sm mt-1">
              All missions complete. Claim your reward!
            </div>
            <button
